...
- `EV_VitoriaCorrida` (CE 19) é o dispatcher canônico de fim de corrida. Ele calcula `VAR_VITORIA_PASSOU`, limpa reserva de Common Event, desliga o lifecycle da corrida e transfere para o mapa de resultado correto.
- `Map018`, `Map019` e `Map020` são os owners atuais das telas de resultado. Durante essas telas, setas e CEs de gameplay devem respeitar `SW_INPUT_LOCKED`; apenas a confirmação por OK ou clique/toque deve operar.
...
- Retry não deve repetir a VN nem o preload completo quando o caminho validado pula esse trecho por tentativa. O handoff aprovado usa `SW_RACE_RETRY_PENDING` (switch 106) para voltar a `Map001`, desligar o switch e chamar `EV_RaceOrchestrator` (CE 5). Confirme a semântica de `VAR_ATTEMPT_N` antes de mudar o fluxo.
...
| 7 | `EV_RaceRenderer` | Renderiza cena, detecta fim de corrida e chama o dispatcher de resultado. | Não deve pular `EV_VitoriaCorrida` quando `VAR_SCENE_INDEX >= VAR_RACE_N_CENAS`. |
...
| 19 | `EV_VitoriaCorrida` | Dispatcher de resultado: limpa pictures/audio, calcula `VAR_VITORIA_PASSOU`, registra log, desliga `SW_RACE_ACTIVE` e transfere para `Map018`, `Map019` ou `Map020`. | Não deve voltar a renderizar UI própria nem chamar retry direto; os mapas de resultado são owners da confirmação. |

## Mapas de resultado

| Mapa | Evento | Caso | Pós-confirmação |
| --- | --- | --- | --- |
| `Map018` | `Resultado Vitoria` | Vitória por threshold. | Aguarda OK/clique, limpa resultado/audio e transfere para o destino narrativo da corrida atual (`Map005`, `Map013` ou `Map016`). |
| `Map019` | `Resultado Derrota Risk` | Derrota por jogada arriscada falha/crash. | Aguarda OK/clique, limpa resultado/audio, seta `SW_RACE_RETRY_PENDING` e transfere para `Map001`. |
| `Map020` | `Resultado Derrota Posicao` | Derrota por pontuação/posição abaixo do threshold. | Aguarda OK/clique, limpa resultado/audio, seta `SW_RACE_RETRY_PENDING` e transfere para `Map001`. |

## Tela de resultado

Fluxo aprovado em Playtest humano:

1. `EV_RaceRenderer` chama `EV_VitoriaCorrida` ao terminar as cenas.
2. `EV_VitoriaCorrida` faz cleanup defensivo, calcula `VAR_VITORIA_PASSOU` com os thresholds atuais e escolhe o mapa de resultado:
   - vitória -> `Map018`;
   - derrota por `SW_CRASH_FLAG` -> `Map019`;
   - derrota por pontuação/posição -> `Map020`.
3. O mapa de resultado toca a ME adequada, mostra TextPictures e aguarda `Input.isTriggered('ok')` ou `TouchInput.isTriggered()`.
4. Ao confirmar, o mapa limpa pictures/audio/locks e executa a rota pós-resultado.

Textos MVP aprovados:

- `Map018`: `\C[6]VITORIA!`, `Glory Score: \V[105]`, `Pressione [ESPACO] ou clique`.
- `Map019`: `\C[18]DERROTA!`, `Jogada arriscada falhou`, `Pressione [ESPACO] ou clique`.
- `Map020`: `\C[18]DERROTA!`, `Voce nao chegou em primeiro`, `Pressione [ESPACO] ou clique`.

O Playtest humano validou as três rotas de resultado e ausência de vazamento de áudio nesse fluxo.
...
- A confirmação da tela usa `Input.isTriggered('ok')` ou `TouchInput.isTriggered()`.
- Ao sair, apague as pictures de resultado de forma defensiva.

## Retry e tela preta

Fluxo de retry aprovado:

1. `Map019` ou `Map020` seta `SW_RACE_RETRY_PENDING` (switch 106) e transfere para `Map001`.
2. A página autorun prioritária de `Init Corrida` em `Map001` exige esse switch, desliga `SW_RACE_RETRY_PENDING`, chama `EV_RaceOrchestrator` (CE 5) e executa `Erase Event`.
3. Esse handoff reinicia a mesma corrida sem repetir VN nem depender do evento local apagado anteriormente.
...
- `SW_RACE_ACTIVE`, `SW_INPUT_LOCKED`, `SW_CRASH_FLAG`, `SW_RACE_RETRY_PENDING`.
