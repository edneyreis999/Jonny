...
| Plugin/no-plugin drift | Older implementation text says "sem plugins"; actual runtime uses `TextPicture`, `ButtonPicture`, `Jhonny_RaceHelper`, `VisuMZ_0_CoreEngine` and `VisuMZ_2_VNPictureBusts`. Localization changes observed on 2026-07-06 also activate `VisuMZ_1_MessageCore`, Options/Save Core and Message Core extension plugins. | Open. Future implementation must treat plugins as active runtime dependencies unless a refactor is explicitly approved. |
...
| Language/copy drift | System locale is `pt_BR`; docs and UI terms are mostly Portuguese, but some runtime result TextPictures and route terms appear in English. On 2026-07-06, PT/EN localization was partially implemented with Message Core, `Languages.csv` and `\tl{...}` references in maps/System. | Partially addressed statically. Requires Playtest, LQA and product/localization review before closing. |
