...
0. Definição do arquivo de saída

Antes de escrever a retrospectiva, determine onde o documento deve ser salvo.

1. Verifique se o usuário já informou nesta sessão:
    * O diretório de destino;
    * O caminho completo do arquivo; ou
    * Uma convenção do projeto para armazenar retrospectivas, post-mortems ou documentos de melhoria contínua.
2. Caso o destino já esteja definido e seja inequívoco:
    * Não pergunte novamente;
    * Use o local informado;
    * Informe brevemente o caminho que será utilizado.
3. Caso o destino não esteja definido, interrompa a execução antes de produzir o documento e pergunte ao usuário:
    Em qual diretório e com qual nome você quer que eu salve o arquivo da retrospectiva técnica?
4. Quando for útil, sugira um nome de arquivo baseado na tarefa executada, utilizando um formato consistente, por exemplo:
    retrospectiva-<nome-da-tarefa>.md
5. Não escolha silenciosamente um diretório apenas por parecer apropriado.
6. Não crie o arquivo enquanto o destino estiver ambíguo.
7. Após receber a resposta:
    * Confirme que o diretório existe ou crie-o, caso tenha permissão;
    * Gere a retrospectiva completa;
    * Salve o conteúdo no caminho definido;
    * Informe ao usuário o caminho final do arquivo.
8. Caso o ambiente não permita criar ou salvar arquivos:
    * Informe claramente essa limitação;
    * Ainda assim produza o documento no chat;
    * Não afirme que o arquivo foi salvo.

A pergunta sobre o destino deve ser feita apenas uma vez. Não solicite confirmação adicional quando o usuário tiver fornecido um caminho válido e inequívoco.
