...
### Patch F — Create or extend EV_UpdateHud
...
```jsonc
{
  ...
  "list": [
    // Label: HUD_TICK
    {code: 0, indent: 0, parameters: ["HUD_TICK"]},  // code 0 = label? verify
    ...
    // Erase picture
    {code: 232, indent: 0, parameters: [<HUD_ID>]},  // code 232 = erase picture
...
### Patch G — Add Show Picture to EV_RaceOrchestrator INIT
...
Use the same coordinates and picture ID as Patch F so the two CEs render at
the same location.
...
5. Create `Jhonny/planos/003-bug-fix-round1/fase3/build_phase3_ces.py` with
   `patch_f_create_update_hud(ces)` and `patch_g_init_reshow(ces)`. Both
...
- [ ] Patch F template references a CE slot that is currently empty.
- [ ] Patch F uses exact RMMZ codes from an existing TextPicture CE.
- [ ] Patch G inserts after `SW_RACE_ACTIVE = ON` in CE 5.
