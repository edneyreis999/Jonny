...
MAP_PATH = PROJECT_DIR / "data" / "Map005.json"
...
def find_marker(commands):
    ...
    raise AssertionError("Race 2 marker with transfer to Map013 was not found")
...
def validate_post_patch(path):
    ...
    for index, command in enumerate(commands[:-1]):
        ...
        assert next_command["parameters"][1] == TARGET_MAP_ID, (
            "Race 2 marker does not transfer to Map001"
        )
        return {
            "eventId": event["id"],
    ...
    raise AssertionError("Race 2 marker with transfer to Map001 was not found")
